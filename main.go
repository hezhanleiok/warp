...
// 补全 Noise_IKpsk2 必需的 KDF3 函数
func kdf3(ck []byte, ikm []byte) ([]byte, []byte, []byte) {
	prk := hmacBlake2s256(ck, ikm)
	t1 := hmacBlake2s256(prk, []byte{0x01})
	t2 := hmacBlake2s256(prk, append(append([]byte(nil), t1...), 0x02))
	t3 := hmacBlake2s256(prk, append(append([]byte(nil), t2...), 0x03))
	return t1, t2, t3
}

// 核心修复：严谨包含 Noise_IKpsk2 标准的 PSK 派生流程，确保对端解密时间戳成功
func createRealWireGuardHandshake(clientPriv [32]byte, clientPub [32]byte, peerPub [32]byte) ([]byte, uint32, error) {
	...
	// 3. PSK 密钥派生步骤（解决 Cloudflare 认证失败的核心修复）
	var psk [32]byte // 标准 WARP 的预共享密钥为 32 字节全零
	var tempH []byte
	var key3 []byte
	chainingKey, tempH, key3 = kdf3(chainingKey, psk[:])
	h = hash256(append(h[:], tempH...))

	// 4. 使用派生出的 key3 和更新后的 h 正确加密 TAI64N 时间戳
	tai64n := make([]byte, 12)
	now := time.Now().UTC()
	...
	aead2, err := chacha20poly1305.New(key3)
	if err != nil {
		return nil, 0, err
	}
	var nonce2 [12]byte
...
func (a *App) RunWarpScoutFullEngine(account *WarpAccount, maxCount int) ([]EndpointResult, error) {
	...
	// 将所有官方端口均衡打散分配到候选 IP 上，确保每个端口都有充分的测试样本
	for i, ip := range candidateIPs {
		port1 := warpAllOfficialPorts[i%portLen]
		port2 := warpAllOfficialPorts[(i+7)%portLen]
		port3 := warpAllOfficialPorts[(i+17)%portLen]

		taskList = append(taskList, task{ip, port1})
		taskList = append(taskList, task{ip, port2})
		taskList = append(taskList, task{ip, port3})
	}

	total := len(taskList)
	a.sendLog(fmt.Sprintf("🚀 载入端点池，跨 54 个官方开放端口执行 Noise_IKpsk2 握手测速: %d 个目标 (预计 30~50 秒)...", total))
	...
	a.sendLog(fmt.Sprintf("✔ 探测完成！经 Noise_IKpsk2 完整校验，捕获真实活端点: %d 个", len(validList)))
